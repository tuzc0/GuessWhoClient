...
{
    public partial class UpdateProfileView : UserControl, INotifyPropertyChanged
    {
        ...
        private void OnChangePasswordButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                changePasswordView.PasswordChangeDialogClosed -= dialogClosedHandler;
                HandlePasswordDialogClosed(gameWindow, changePasswordView, accepted);
            };

            changePasswordView.PasswordChangeDialogClosed += dialogClosedHandler;
            gameWindow.ShowScreen(changePasswordView);
        }

        private void HandlePasswordDialogClosed(GameWindow gameWindow, ChangePasswordView changePasswordDialog, bool accepted)
        {
            try
            {
        ...
        private void OnChangeAvatarButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            var chooseAvatarView = new ChooseAvatarView();

            EventHandler<bool> avatarSelectedHandler = null;
            avatarSelectedHandler = (s, accepted) =>
            {
                chooseAvatarView.AvatarChangeClose -= avatarSelectedHandler;
                HandleAvatarDialogClosed(gameWindow, chooseAvatarView, accepted);
            };

            chooseAvatarView.AvatarChangeClose += avatarSelectedHandler;
            gameWindow.ShowScreen(chooseAvatarView);
        }

        private void HandleAvatarDialogClosed(GameWindow gameWindow, ChooseAvatarView chooseAvatarWindow, bool accepted)
        {
            try
            {
