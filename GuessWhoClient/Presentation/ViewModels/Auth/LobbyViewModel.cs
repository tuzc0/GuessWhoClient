...
namespace GuessWhoClient.ViewModels
{
    public sealed class LobbyViewModel : INotifyPropertyChanged
    {
        ...
        public ObservableCollection<LobbyPlayer> LobbyPlayers { get; }
        ...
        public LobbyViewModel(GamePlayParameters gamePlayParameters, IMatchSessionController sessionController)
        {
            ...
            LobbyPlayers = new ObservableCollection<LobbyPlayer>(
                gamePlayParameters.Players ?? Array.Empty<LobbyPlayer>());
        ...
        private void OnReadyChanged(LobbyPlayer player)
        {
            if (player == null)
            {
                return;
            ...
            App.Current.Dispatcher.Invoke(() =>
            {
                ...
                {
                    if (LobbyPlayers[index].UserId == player.UserId)
                    {
                ...
                if (indexToReplace >= 0)
                {
                    LobbyPlayers[indexToReplace] = player;
                }
                else
                {
                    Logger.WarnFormat(
